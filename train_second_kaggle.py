...
def freeze_module(model, module_name, freeze):
     """Freezes or unfreezes a module's parameters."""
...
def main(config_path):
    ...
    for epoch in range(start_epoch + 1, epochs):
        ...
        if epoch < 2000: # Example: Train decoder only after epoch 50
            ...
            freeze_module(model, 'bert_encoder', False) # Unfreeze
            freeze_module(model, 'bert', False) # Unfreeze
            freeze_module(model, 'predictor', False) # Unfreeze
            freeze_module(model, 'text_aligner', False) # Unfreeze
            freeze_module(model, 'pitch_extractor', False) # Unfreeze
            freeze_module(model, 'wd', False) # Unfreeze
            freeze_module(model, 'style_encoder', False) # Unfreeze
            freeze_module(model, 'text_encoder', False) # Unfreeze
            freeze_module(model, 'predictor_encoder', False) # Unfreeze
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            ss = []
            gs = []
            print("len mel_input_length: ",len(mel_input_length))
            ...
            # s_dur = torch.cat(ss, dim=0)  # global prosodic styles
            ...
            print("mel_input_length: ",mel_input_length.min().item())
            ...
            with torch.no_grad():
                ...
                if epoch >= joint_epoch:
                    ...
                    wav = y_rec_gt # use reconstruction since decoder is fixed
                    # wav = y_rec_gt # use recording since decoder is tuned
        ...
        if epoch % saving_epoch == 0:
            ...
            wave, sr = librosa.load("/kaggle/working/StyleTTS2/Dataset/audio_000074.wav", sr=24000)
            ...
            ref_s = torch.cat([ref_s, ref_p], dim=1)

            # g2p = vi.VIG2P(dialect="south")
            text = "ɛm1 biək5 ɛm1 kɔŋ͡m2 ɲiəw2 tʰiəw5 ʂɔk͡p5 lăm5 , ɲɤ̆k5 la2 fɤ̆ŋ2 fat5 ɤ̆m1 vɤj5 ciɲ5 ta1 . taj6 ɛm1 mɤj5 dɯək6 ʈaj1 kɔ5 mɤ̆j5 ŋăj2 tʰoj1 , kɔŋ͡m2 ɲiəw2 tʰɯ5 faj4 hɔk͡p6 . ɛm1 ʂɛ3 ko5 ɣăŋ5 kaj4 tʰiəŋ6 de4 nɔj5 cwʷiən6 tɯ6 ɲiəŋ1 va2 cwʷɤ̆n4 sak5 hɤŋ1 . ci6 tʰɤ̆j5 ɛm1 hăj1 dɔk͡p6 ʂaj1 ɤ4 co3 naw2 xoŋ͡m1 a6 ? ci6 ɣɔp5 i5 tʰem1 cɔ1 ɛm1 vɤj5 ."
            ps = text.strip()
            # ps, _ = g2p(text)
            # ps = word_tokenize(ps)
            ...
            with torch.no_grad():
                ...
                # Assuming 'out' is your audio numpy array and sr=24000
                sample_rate = 24000
                output_path = osp.join(log_dir, 'epoch_2nd_%05d.wav' % epoch)

                # soundfile handles the conversion internally
